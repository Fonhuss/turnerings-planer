...
import { Plus, Copy, Trash2, Play, ArrowRight, ListOrdered, Brackets } from "lucide-react";
...
export default function TournamentPlanner() {
  ...
  const [leagueLocked, setLeagueLocked] = useState<boolean>(false); // NEW: lås liga ved overgang til knockout
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        ...
        setLog(s.log ?? "");
        setLeagueLocked(s.leagueLocked ?? false);
      } catch {}
    }
  }, []);

  useEffect(() => {
    const snapshot = { type, comboPhase, teams, leagueCycles, leagueMatches, roundNames, legsPerRound, koMatches, koStructureReady, includeConsolation, log, leagueLocked };
    localStorage.setItem("tourneyState-v1", JSON.stringify(snapshot));
  }, [type, comboPhase, teams, leagueCycles, leagueMatches, roundNames, legsPerRound, koMatches, koStructureReady, includeConsolation, log, leagueLocked]);
  ...
  const startKnockoutFromLeague = (numQualifiers: number, lockLeague: boolean) => {
    ...
    if (lockLeague) setLeagueLocked(true);
  };
        ...
        <Card className="shadow-sm">
          <CardContent className="p-4 space-y-4">
            <div className="flex flex-wrap items-end gap-3">
              ...
              <Button onClick={generateLeague} disabled={leagueLocked}><Play className="mr-1" size={16}/> Generér kampprogram</Button>
              {leagueLocked && <Badge variant="secondary">Liga låst</Badge>}
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              {comboPhase === 'league' && (
                <div className="flex items-end gap-3">
                  ...
                  <Button onClick={generateLeague} disabled={leagueLocked}><Play className="mr-1" size={16}/> Generér liga</Button>
                  {leagueLocked && <Badge variant="secondary">Liga låst</Badge>}
                </div>
              )}
            </div>
            ...
            {comboPhase === 'league' && (
              <Tabs defaultValue="schedule">
                ...
                <TabsContent value="advance" className="space-y-4">
                  <RoundConfig roundNames={roundNames} setRoundNames={setRoundNames} legsPerRound={legsPerRound} setLegsPerRound={setLegsPerRound} />
                  <AdvanceFromLeague leagueTable={leagueTable} onAdvance={(n, lock) => startKnockoutFromLeague(n, lock)} />
                </TabsContent>
              </Tabs>
            )}
...
function AdvanceFromLeague({ leagueTable, onAdvance }: { leagueTable: { team: Team; pts: number; gf: number; ga: number; gd: number }[]; onAdvance: (n: number, lock: boolean) => void; }) {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-end gap-3">
        ...
        <Button onClick={() => onAdvance(n, lock)}><ArrowRight size={16} className="mr-1"/> Afslut liga og start knockout</Button>
      </div>
      <div className="flex items-center gap-2 text-sm text-muted-foreground">
        <Switch id="lockleague" checked={lock} onCheckedChange={setLock} />
        <Label htmlFor="lockleague">Lås liga-stilling ved start af knockout</Label>
      </div>
